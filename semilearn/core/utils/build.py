...
def get_net_builder(net_name, from_name: bool):
    ...
    if from_name:
        ...
        model_name_list = sorted(name for name in nets.__dict__ if name.islower() and not name.startswith("__") and callable(nets.__dict__[name]))
...
def get_dataset(args, alg, dataset, num_labels, data_dir="./data", include_lb_to_ulb=True):
    ...
    if dataset in datasets_dict["cv"]:
        ...
        raise ValueError(
            f"Invalid dataset: '{dataset}'. Valid datasets include {registered_datasets}. "
            f"Please modify the dataset or register '{dataset}' in ./semilearn/core/utils/build.py."
        )
...
def get_cosine_schedule_with_warmup(optimizer, num_training_steps, num_cycles=7.0 / 16.0, num_warmup_steps=0, last_epoch=-1):
    ...
    # return MultiStepLR(optimizer, milestones=[50], gamma=0.75)


def get_criterion(criterion_name="mse"):
    """
    return criterion (name) in torch.nn.functional.
    """
    if criterion_name == "l1":
        ...
        raise ValueError(f"{criterion_name} is not an valid criterion name.")
